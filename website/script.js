...
function placeCards() {
  ...
  const columnPositions = [6, 24, 42, 60, 78];
  const totalRows = Math.ceil(reasons.length / columnPositions.length);
  const rowHeight = 145;
  board.style.height = `${totalRows * rowHeight + 40}px`;
  ...
  shuffledReasons.forEach((reason, index) => {
    ...
    const col = index % columnPositions.length;
    const row = Math.floor(index / columnPositions.length);
    const x = columnPositions[col] + (index % 2 === 0 ? -2 : 3);
    const y = row * rowHeight + (index % 3 === 0 ? 8 : 0);
